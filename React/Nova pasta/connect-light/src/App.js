...
export class MapContainer extends Component {
  ...
  // displayAddress = (option, value,raio) => {
  //   this.doPositioning(option, value);
  //   this.textInput.current.value = value;
  //   this.textRaio.current.value = raio;
          ...
          .then((responseJson) => {
            const address1 = responseJson.results[0].formatted_address;
              this.setState({
                ...
                showRaio: {result: response.data[0].R},
  ...
  render() {
   ...
   for(let i =0;i < ;i++){
    markers.push[i](<Marker position={{lat: this.state.posx[i].result, lng: this.state.posy[i].result}} onClick={this.onMarkerClick}/>)
   }
    Moment.locale('pt-BR');
